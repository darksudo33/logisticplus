...
function stepStateForShipment(shipment: any, order: number) {
  const profiles = {
    ...
    KOOTAJ_DONE: { completeThrough: 5, current: 6 },
    EXITED: { completeThrough: 6, current: -1 },
    LOADING: { completeThrough: 0, current: 1 },
  };
  const profile = profiles[shipment.status] || profiles.LOADING;
