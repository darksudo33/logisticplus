...
export const useAppStore = create<MockStore>((set) => ({
  ...
  loadCurrentUserRecords: async () => {
    ...
    useAppStore.setState({ isHydratingFromDatabase: true });
    try {
      ...
      if (!authResponse.ok) {
        ...
        if (authResponse.status === 401 || authResponse.status === 403) {
          ...
          useAppStore.setState({ isHydratingFromDatabase: false });
        }
        throw error;
      }
      ...
      useAppStore.setState({ currentUser: restoredUser });
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          ...
          useAppStore.setState({ hasHydratedFromDatabase: true, isHydratingFromDatabase: false });
        }
        throw error;
      }
      const payload = await response.json();
      useAppStore.getState().hydrateFromRecords(payload.records || {});
      useAppStore.getState().refreshNotifications().catch(logBackgroundNotificationRefreshError);
    } catch (error) {
  ...
  refreshUsers: async () => {
    ...
    const payload = await response.json();
    suppressNextDatabaseSave = true;
    useAppStore.setState({ users: payload.data || [] });
  },
  ...
  refreshDocuments: async () => {
    ...
    const payload = await response.json();
    useAppStore.setState({ documents: payload.data || [] });
  },
  ...
  refreshCustomers: async () => {
    ...
    const payload = await response.json();
    useAppStore.setState({ customers: payload.data || [] });
  },
  ...
  refreshShipments: async () => {
    ...
    const payload = await response.json();
    suppressNextDatabaseSave = true;
    useAppStore.setState({ shipments: (payload.data || []).map(normalizeShipmentRecord) });
  },
  ...
  refreshTasks: async () => {
    ...
    const payload = await response.json();
    suppressNextDatabaseSave = true;
    useAppStore.setState({ tasks: (payload.data || []).map(normalizeTaskRecord) });
  },
  ...
  fetchOrganizationMembers: async () => {
    ...
    useAppStore.setState({ organizationMembers: data });
    return data;
  },
  ...
  refreshNotifications: async () => {
    ...
    const payload = await response.json();
    suppressNextDatabaseSave = true;
    useAppStore.setState({ notifications: payload.data || [] });
  },
  ...
  addShipment: async (shipment) => {
    ...
    const newSteps = buildShipmentWorkflowSteps(id, useAppStore.getState().defaultSteps);
    ...
    await useAppStore.getState().refreshShipments();
  },
  ...
  markNotificationRead: async (id) => {
    const previousNotifications = useAppStore.getState().notifications;
    suppressNextDatabaseSave = true;
    set((state) => ({
    ...
    if (!response.ok) {
      suppressNextDatabaseSave = true;
      useAppStore.setState({ notifications: previousNotifications });
...
async function persistCurrentStateNow() {
  ...
  await saveStateToDatabase(useAppStore.getState());
}
