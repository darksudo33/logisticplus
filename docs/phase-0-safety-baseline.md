...
Logistic Plus is a Persian/RTL B2B logistics SaaS with a React/Vite frontend, Express/Node backend, PostgreSQL through raw `pg` queries, local filesystem document storage, multi-tenant organizations, public tracking, documents, shipment workflow, billing/Zarinpal, SMS, platform admin, and Playwright E2E coverage.

Main backend entry points:

- `server.js` is the composition root. It creates the Express app, runs startup checks, registers inline and extracted API route groups, serves Vite/static assets, handles WebSocket chat, starts the SMS worker, and owns the Zarinpal request/verify orchestration.
...
- `src/server/db.js` is still the main raw `pg` data-access and compatibility layer. It exports the shared `pool`, auth/session helpers, raw query functions, UI DTO mappers, write/archive helpers, billing helpers, SMS helpers, and compatibility bridge functions.
...
Billing/Zarinpal routes:

- `server.js` owns `/api/billing/payments/:id/start`, `/api/billing/zarinpal/callback`, `/api/billing/my-subscription`, `/api/billing/my-invoices`, and `/api/billing/my-payments`.
- `src/server/repositories/billing.js` owns the idempotent callback transition with row locking, invoice close, receipt creation, signup/org/subscription status updates, and subscription events.
- Platform admin billing routes remain inline in `server.js`.
...
| Billing/Zarinpal | `server.js`, `src/server/repositories/billing.js` | Callback idempotency, amount verification, and status transitions can double-charge, double-receipt, or corrupt signup state. | Keep callback transitions transactional and covered by focused tests. |
...
- Extracted repositories: `src/server/repositories/billing.js`, `customers.js`, `documents.js`, `notifications.js`, `shipment-progress.js`, `shipments.js`, `users.js`.
...
- Live production Zarinpal validation is documented as pending in existing handoff docs; sandbox coverage is not the same as a live merchant payment smoke.
...
2. Add focused regression tests before changing guards: cross-tenant denial for protected APIs, public DTO leak tests, document storage-key leak tests, and billing callback idempotency tests.
