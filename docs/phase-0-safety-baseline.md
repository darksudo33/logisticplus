...
| Compatibility bridge | `src/server/db.js`, `src/store/useAppStore.ts`, `/api/users/:id/bootstrap` | Removing or bypassing bridge behavior can break legacy screens and data sync. | Do not remove `user_records` or bootstrap until intentionally retired. |
