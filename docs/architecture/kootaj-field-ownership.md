...
| Shipment status | `shipment.status` | Shipment canonical status | Defer for initial Phase 2 | Yes, via Shipment Detail base status | Yes, via Daily Status `baseInfo.status` | No | Yes | Yes | Status can affect workflow. Keep out of the first editable Kootaj subset unless status-transition rules are reused exactly. |
...
Shipment status should also remain out of the first editable Kootaj subset because existing status changes can affect workflow expectations. If it is added later, it must use the exact same transition path as Daily Status/Shipment Detail.
